...
def analyze_market(client: anthropic.Anthropic, df: pd.DataFrame, ticker: dict, balance: dict) -> dict:
    ...
    system_prompt = (
        ...
        "  order_price: float — suggested LIMIT order price (for buy: near support below current price; for sell: near resistance above; for hold: use current price)\n"
        "  tp_price: float — take-profit price target based on nearest resistance / measured move\n"
        "  sl_price: float — stop-loss price based on nearest support / invalidation level\n"
        "  delta_volume: float — suggested quantity in base currency (e.g. BTC) based on risk and confidence\n\n"
